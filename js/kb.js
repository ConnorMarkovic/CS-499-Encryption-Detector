// ═══════════════════════════════════════════════════════════════════════
//  kb.js — Knowledge Base + Feature Descriptions + Tooltips
//  Dependencies: ciphers.js (ENGLISH_FREQ), ml.js (mlModel)
// ═══════════════════════════════════════════════════════════════════════

// ═══════════════════════════════════════════════════════════════════════
//  KNOWLEDGE BASE (persisted via KBStore in storage.js)
// ═══════════════════════════════════════════════════════════════════════

const SEED_KB={
  caesar:{name:'Caesar Cipher',cat:'monoalphabetic_substitution',desc:'Each letter shifted by a fixed amount. 25 possible keys.',enc:'Choose shift 1-25. Each letter moves forward by that amount, wrapping Z→A.',dec:'Try all 25 shifts, score each against English letter frequencies. Chi-squared scoring automates this.',ic:[.060,.070],weakness:['Only 25 keys — trivially brute-forced','Preserves letter frequency distribution','Single-letter words reveal themselves'],hist:'Used by Julius Caesar ~50 BCE.'},
  vigenere:{name:'Vigenère Cipher',cat:'polyalphabetic_substitution',desc:'Keyword-based polyalphabetic cipher. Each letter shifted by a different key letter.',enc:'Repeat keyword to match plaintext length. Shift each letter by the key letter position.',dec:'Kasiski examination: find repeated trigrams, GCD of spacings = key length. Friedman IC test confirms. Split into Caesar streams, solve each.',ic:[.038,.055],weakness:['Kasiski reveals key length','Short keys especially vulnerable','Reduces to multiple Caesar ciphers'],hist:'Described by Bellaso 1553. Called "le chiffre indéchiffrable" for 300 years.'},
  substitution:{name:'Simple Substitution',cat:'monoalphabetic_substitution',desc:'Each letter mapped via fixed permutation. 26! ≈ 4×10²⁶ keys.',enc:'Create a random permutation of the alphabet as the key.',dec:'Frequency analysis + hill-climbing: swap letter pairs, score against n-gram frequencies.',ic:[.060,.070],weakness:['Preserves frequency distribution','Hill-climbing solves texts >50 chars'],hist:'Used since antiquity.'},
  atbash:{name:'Atbash Cipher',cat:'monoalphabetic_substitution',desc:'Alphabet mirror: A↔Z, B↔Y. Self-inverse.',enc:'Replace each letter with its mirror. No key.',dec:'Apply same transformation. Check if result is English.',ic:[.060,.070],weakness:['Only one possible key','Self-inverse'],hist:'Hebrew cipher from the Bible.'},
  enigma:{name:'Enigma Machine',cat:'electromechanical_polyalphabetic',desc:'3-rotor polyalphabetic cipher with plugboard. ~10¹⁸ configurations.',enc:'Configure 3 rotors, positions, ring settings, plugboard. Signal passes through rotors and reflector.',dec:'Self-inverse with same settings. Without: Bombe-style attack exploiting no-self-encryption.',ic:[.038,.042],weakness:['No letter encrypts to itself','Reflector reduces keyspace','Double-stepping anomaly'],hist:'Nazi Germany WWII. Cracked by Turing at Bletchley Park.'},
  affine:{name:'Affine Cipher',cat:'monoalphabetic_substitution',desc:'E(x) = (ax + b) mod 26. 312 possible keys.',enc:'Choose a (coprime to 26) and b (0-25).',dec:'Brute-force 312 combinations.',ic:[.060,.070],weakness:['Only 312 keys','Preserves frequency distribution'],hist:'Generalization of Caesar.'},
  rail_fence:{name:'Rail Fence Cipher',cat:'transposition',desc:'Zigzag text across N rails, read horizontally.',enc:'Choose rails. Write zigzag, read each rail.',dec:'Try all rail counts 2-N/2, score against English.',ic:[.060,.070],weakness:['Frequencies unchanged','Tiny keyspace'],hist:'American Civil War.'},
  playfair:{name:'Playfair Cipher',cat:'digraph_substitution',desc:'Encrypts letter pairs via 5×5 key matrix.',enc:'Build matrix from keyword. Same row: shift right. Same column: shift down. Rectangle: swap.',dec:'Reverse rules. Simulated annealing on digraph frequencies.',ic:[.045,.060],weakness:['Only 25 chars (I=J)','Digraph patterns preserved'],hist:'Wheatstone 1854, Boer War and WWI.'},
  xor_single:{name:'XOR Single-Byte',cat:'modern_broken',desc:'XORs every byte with one key byte. 256 keys. Used in malware obfuscation.',enc:'XOR each byte with key byte (0-255). Self-inverse.',dec:'Brute-force all 256 keys in milliseconds. One known byte reveals the key.',ic:[.055,.068],weakness:['256 keys — trivial','Known plaintext reveals key','Preserves byte frequency patterns'],hist:'Ubiquitous in malware and CTF challenges.'},
  xor_repeating:{name:'XOR Repeating-Key',cat:'modern_broken',desc:'XORs with repeating multi-byte key. The byte-level Vigenère. Basis of WEP.',enc:'Repeat key to plaintext length. XOR each byte.',dec:'Hamming distance finds key length. Split into single-byte streams, crack each.',ic:[.040,.058],weakness:['Hamming distance reveals key length','Key reuse is catastrophic: C₁⊕C₂ = P₁⊕P₂'],hist:'WEP WiFi was broken 2001 via related attack on RC4.'},
  rc4:{name:'RC4 Stream Cipher',cat:'modern_broken',desc:'Stream cipher: KSA builds permutation from key, PRGA generates keystream XORed with plaintext.',enc:'KSA initializes 256-byte S-box. PRGA steps indices, swaps, outputs keystream bytes.',dec:'Short keys (1-2 bytes) brute-forced. Second output byte biased toward zero (p=1/128).',ic:[.038,.045],weakness:['Second byte biased toward zero','Initial bytes have detectable biases','Fluhrer-Mantin-Shamir broke WEP','Banned from TLS (RFC 7465, 2015)'],hist:'Rivest 1987. Leaked 1994. Once 30% of TLS. Banned 2015.'},
  beaufort:{name:'Beaufort Cipher',cat:'polyalphabetic_substitution',desc:'Reciprocal Vigenère: C = (K - P) mod 26. Self-inverse.',enc:'For each letter: ciphertext = (Key - Plaintext) mod 26.',dec:'Same formula (self-inverse). Kasiski/Friedman attacks work identically to Vigenère.',ic:[.038,.055],weakness:['Same as Vigenère','Self-inverse reveals decryption'],hist:'Named after Sir Francis Beaufort.'},
  porta:{name:'Porta Cipher',cat:'polyalphabetic_substitution',desc:'Self-reciprocal using 13 alphabets. Key pairs (AB,CD,...) select alphabets.',enc:'Key pairs select from 13 alphabets swapping A-M with N-Z.',dec:'Similar to Vigenère cracking but effective key length is halved.',ic:[.042,.058],weakness:['Key pairs halve effective key length','Kasiski-style attacks work'],hist:'Giovanni della Porta, 1563.'},
  columnar:{name:'Columnar Transposition',cat:'transposition',desc:'Writes into rows under keyword, reads columns in alphabetical keyword order.',enc:'Write rows of keyword width. Read columns by sorted keyword order.',dec:'Try column count permutations. Letter frequencies unchanged = transposition giveaway.',ic:[.060,.070],weakness:['Frequencies unchanged','Limited column permutations'],hist:'WWI and WWII. Double transposition used by SOE.'},
  rot47:{name:'ROT47',cat:'modern_obfuscation',desc:'Extends ROT13 to all printable ASCII (33-126), rotating by 47.',enc:'Each char code 33-126: new = 33 + ((code-33+47) mod 94). Self-inverse.',dec:'Apply same transform. Trivially detected.',ic:[.055,.068],weakness:['No key','Self-inverse','Easily detected'],hist:'Extension of ROT13. Config file obfuscation.'},
  binary:{name:'Binary Encoding',cat:'encoding',desc:'Represents each character as its 8-bit binary (base-2) value. Each byte becomes 8 zeros and ones.',enc:'Convert each character to its ASCII code, then to an 8-bit binary string. Pad with leading zeros to 8 digits. Separate bytes with spaces.',dec:'Split on spaces, parse each 8-bit group as base-2, convert to character. Immediately recognizable by containing only 0s, 1s, and spaces in groups of 8.',ic:[0,0],weakness:['Not encryption — trivially reversible','Instantly recognizable pattern (only 0s and 1s)','No key or secret involved'],hist:'Fundamental data representation. Used in computing since the 1940s.'},
  hex:{name:'Hexadecimal Encoding',cat:'encoding',desc:'Represents each byte as two hexadecimal (base-16) digits using characters 0-9 and a-f.',enc:'Convert each character to its ASCII code, then to a 2-digit hex string (00-FF).',dec:'Parse each pair of hex characters as base-16, convert to character. Recognizable by even-length strings containing only 0-9 and a-f.',ic:[0,0],weakness:['Not encryption — trivially reversible','Even-length hex-only strings are easily detected','Commonly seen in memory dumps, color codes, and packet captures'],hist:'Standard representation for binary data since early computing. Used in HTML colors (#FF0000), MAC addresses, and debugging.'},
  base64:{name:'Base64 Encoding',cat:'encoding',desc:'Encodes binary data into printable ASCII using a 64-character alphabet (A-Z, a-z, 0-9, +, /) with = padding.',enc:'Split input into 3-byte groups. Convert each group to four 6-bit indices. Map each index to the Base64 alphabet. Pad with = if the input length is not divisible by 3.',dec:'Map each Base64 character back to its 6-bit value. Recombine into 8-bit bytes. Recognized by the character set and trailing = padding.',ic:[0,0],weakness:['Not encryption — trivially reversible','= padding is a strong detection signal','Character set is distinctive (mixed case + digits + /+)','Data expands by ~33%'],hist:'Defined in RFC 4648. Ubiquitous in email (MIME), data URIs, JSON Web Tokens, and API payloads.'},
  ...
  url:{name:'URL Encoding',cat:'encoding',desc:'Encodes special characters as %XX where XX is the hex value. Also called percent-encoding. Spaces become %20 or +.',enc:'Replace each non-alphanumeric character (except - _ . ~) with %XX where XX is its hex ASCII code. Spaces can become %20 or +.',dec:'Replace each %XX sequence with the character at that hex code value. Recognized by frequent % characters followed by two hex digits.',ic:[0,0],weakness:['Not encryption — standard reversible encoding','% followed by hex pairs is instantly recognizable','Alphanumeric characters pass through unchanged, revealing partial plaintext'],hist:'Defined in RFC 3986 for URIs. Essential for web forms, query strings, and any data transmitted in URLs.'},
  ...
  scytale:{name:'Scytale',cat:'transposition',desc:'Ancient Greek transposition. Text wrapped around a cylinder, read off linearly.',enc:'Write text into rows of cylinder-diameter width, read columns.',dec:'Try different diameters. Letter frequencies unchanged.',ic:[.060,.070],weakness:['Small keyspace (column widths)','Frequencies preserved'],hist:'Used by Spartans ~700 BCE. One of the oldest known ciphers.'},
  route_cipher:{name:'Route Cipher',cat:'transposition',desc:'Text written into a grid, read off in a spiral or zigzag route.',enc:'Fill grid row by row. Read in clockwise spiral from top-left.',dec:'Reverse the spiral read pattern.',ic:[.060,.070],weakness:['Frequencies preserved','Grid dimensions limit keyspace'],hist:'Used in American Civil War battlefield communications.'},
  base32:{name:'Base32 Encoding',cat:'encoding',desc:'Encodes binary data using A-Z and 2-7. Case-insensitive. Padding with =.',enc:'Convert bytes to 5-bit groups, map each to A-Z/2-7 alphabet. Pad with = to multiple of 8.',dec:'Remove padding, convert each character to 5-bit value, reconstruct bytes.',ic:[0,0],weakness:['Not encryption — trivially reversible','Uppercase-only output with restricted digit set is distinctive','= padding signals Base32'],hist:'RFC 4648. Used in TOTP tokens, onion addresses, and DNS-safe encodings.'},
  base58:{name:'Base58 Encoding',cat:'encoding',desc:'Alphanumeric encoding excluding 0, O, I, l to avoid visual confusion. No padding.',enc:'Convert bytes to big integer, repeatedly divide by 58, map remainders to alphabet.',dec:'Reverse the base conversion.',ic:[0,0],weakness:['Not encryption','Absence of 0/O/I/l is a detection signal'],hist:'Invented for Bitcoin addresses. Used in IPFS CIDs and Flickr short URLs.'},
  ascii85:{name:'Ascii85 (Base85)',cat:'encoding',desc:'Encodes 4 bytes into 5 ASCII characters (33-117). Higher density than Base64. Wrapped in <~ ~> delimiters.',enc:'Group bytes in 4s, convert to base-85 using characters !-u. Special case: four zeros become "z".',dec:'Reverse the base-85 conversion. Strip <~ ~> wrapper.',ic:[0,0],weakness:['Not encryption','<~ ~> wrapper is instantly recognizable','Unusual punctuation-heavy output is distinctive'],hist:'Used in PostScript and PDF for binary data embedding. Also called btoa encoding.'},
  uuencode:{name:'UUEncode',cat:'encoding',desc:'Legacy Unix encoding. Lines start with length character, use ASCII 32-95. Wrapped in begin/end.',enc:'Each line: length prefix + 4 encoded chars per 3 input bytes. Characters offset by 32.',dec:'Parse line-by-line, subtract 32 from each char, reconstruct 3-byte groups.',ic:[0,0],weakness:['Not encryption','begin/end markers and line structure are obvious','Characters 32-95 only'],hist:'Unix-to-Unix encoding from 1980. Replaced by Base64/MIME but still seen in legacy systems.'},
  html_entities:{name:'HTML Entities',cat:'encoding',desc:'Encodes characters as numeric HTML entities: &#CODE; where CODE is the decimal ASCII value.',enc:'Replace each character with &#CODE; where CODE is its charCodeAt value.',dec:'Parse each &#N; and convert N back to a character.',ic:[0,0],weakness:['Not encryption','&#...; pattern is unmistakable','Semicolons and ampersands make detection trivial'],hist:'Defined in HTML specification. Used to display special characters in web pages safely.'},
  bifid:{name:'Bifid Cipher',cat:'fractionation',desc:'Polybius square + fractionation. Each letter becomes row/column coordinates, coordinates are interleaved, then converted back.',enc:'Map letters to 5×5 grid coordinates. Collect all rows then all columns. Recombine pairs into grid positions.',dec:'Reverse the fractionation process.',ic:[.045,.060],weakness:['Grid-based — 5×5 key matrix has limited keyspace','Period-based patterns can be exploited'],hist:'Invented by Félix Delastelle in 1901.'},
  polybius:{name:'Polybius Square',cat:'encoding',desc:'Each letter becomes a two-digit coordinate (11-55) in a 5×5 grid. I/J merged.',enc:'Map each letter to row-column position in the grid. Output pairs of digits 1-5.',dec:'Parse digit pairs, look up grid positions.',ic:[0,0],weakness:['Not encryption — trivially reversible','Only digits 1-5 appear','Always even-length digit output'],hist:'Invented by the Greek historian Polybius ~150 BCE. Basis for many later ciphers including Bifid and ADFGVX.'},
  ...
  tap_code:{name:'Tap Code',cat:'encoding',desc:'Prison cipher. Each letter becomes a row.column pair (1-5) tapped on walls. K merged with C.',enc:'Map each letter to its 5×5 grid position as R.C format.',dec:'Parse R.C pairs back to grid positions.',ic:[0,0],weakness:['Not encryption — simple grid lookup','K/C merge','Digit pairs 1.1-5.5 are distinctive'],hist:'Used by prisoners of war, especially in Vietnam War POW camps. Based on Polybius square.'},
  phone_keypad:{name:'Phone Keypad',cat:'encoding',desc:'Maps letters to phone keypad numbers: ABC=2, DEF=3, GHI=4, etc. Lossy encoding.',enc:'Replace each letter with its keypad digit (2-9).',dec:'Ambiguous — each digit maps to 3-4 letters. First letter of group is used as approximation.',ic:[0,0],weakness:['Lossy — multiple letters per digit','Not encryption','Digits 2-9 only'],hist:'Based on telephone keypad layout standardized in the 1960s. T9 predictive text disambiguated using dictionaries.'},
  nato_phonetic:{name:'NATO Phonetic Alphabet',cat:'encoding',desc:'Spells each letter as its NATO phonetic word: Alpha, Bravo, Charlie, ..., Zulu.',enc:'Replace each letter with its NATO word, space-separated.',dec:'Map each NATO word back to its letter.',ic:[0,0],weakness:['Not encryption — standard public alphabet','Fixed 26-word vocabulary is instantly recognizable','Word lengths follow a distinctive pattern'],hist:'Adopted by NATO and ICAO in 1956. Used in aviation, military, and radio communications worldwide.'},
  word_sub:{name:'Word Substitution',cat:'word_level_substitution',desc:'Replaces each content word with a random word from a fixed pool. Preserves sentence structure and function words.',enc:'Map each content word to a random replacement from a 50-word pool. Short words (≤2 chars) pass through unchanged.',dec:'Without the word mapping, decryption requires frequency analysis on word patterns.',ic:[.060,.070],weakness:['Sentence structure preserved','Function words unchanged','Word frequency analysis possible'],hist:'Conceptual extension of letter-level substitution to word level. Used in some code languages.'},
...
let KB;
function loadKB(){KB=KBStore.load();
...
function saveKB(){KBStore.save(KB);}
loadKB();
...
function syncAllStats(){
  ...
  // Push to Training page
  if($('tsT'))$('tsT').textContent=totalTested;
  // Push to KB page
...
const FEAT_INFO={
  ...
  // ── NEW FEATURES IN 128-FEATURE MODEL ────────────────────────────
