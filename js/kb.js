...
const SEED_KB={
  ...
  tap_code:{name:'Tap Code',cat:'encoding',desc:'A communication method used by prisoners where each letter is represented by dots. Based on a 5×5 grid (K merges with C), each letter is encoded as a row of dots, a space, then a column of dots — for example A is ". ." and H is ".. ...". Words are separated by " / ". Originally transmitted by tapping on walls.',enc:'Map each letter to its 5×5 grid position. Output row dots, space, column dots. Separate letters with double-space and words with " / ".',dec:'Split on "/" for words, double-space for letters. Count dots in each half to get row and column, look up grid position.',ic:[0,0],weakness:['Not encryption — trivially reversible with the grid','K and C share the same position','Dot groups are visually distinctive','Short letters have very short codes'],hist:'Used by prisoners of war, especially in Vietnam War POW camps. Based on the Polybius square — popularized by U.S. POWs in Vietnam who tapped codes on cell walls.'},
  phone_keypad:{name:'Phone Keypad',cat:'encoding',desc:'Based on old telephone multi-press input where you press a digit key multiple times to cycle through its letters. Key 2 has ABC, so press 2 once for A, twice ("22") for B, three times ("222") for C. Key 7 has PQRS so "7777" = S. Each letter is a space-separated token of repeated digits, so "HI" encodes as "44 444".',enc:'For each letter, find its key digit and position. Repeat that digit position times (1-4). Separate letters by spaces.',dec:'Split on spaces. Each token is a run of one repeated digit — the digit picks the key, the token length picks the letter (wraps if over-pressed).',ic:[0,0],weakness:['Not encryption — completely reversible','Limited charset (letters only, no punctuation)','Groups of repeated digits are immediately recognizable'],hist:'Based on telephone keypad multi-press input (ITU E.161) standardized in the 1990s. The technique predates T9 predictive text and was the original way to type SMS messages.'},
